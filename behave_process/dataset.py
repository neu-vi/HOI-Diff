    ...
    def __init__(self, opt, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list):
            try:
                ...
                obj_points = temp_simp.vertices.astype(np.float32)
                obj_faces = temp_simp.faces.astype(np.float32)
        ...
        if  self.training_stage==1:
            ...
            if self.use_global:
                if mode == 'gt':
                    ...
                    # used by our models
                    self.mean = np.load(pjoin('./dataset/behave', 'Mean_global.npy'))
                    self.std = np.load(pjoin('./dataset/behave', 'Std_global.npy'))
                ...
                if mode == 'gt':
                    ...
                    # used by our models
                    self.mean = np.load(pjoin('./dataset/behave', 'Mean_local.npy'))
                    self.std = np.load(pjoin('./dataset/behave', 'Std_local.npy'))
