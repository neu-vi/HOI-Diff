...
from data_loaders.behave.data.dataset import collate_fn
import codecs as cs
...
class TextMotionMatchTrainer(object):
    ...
    def forward(self, batch_data):
        ...
        word_emb, pos_ohot, caption, cap_lens, motions, m_lens, _, _ ,_ = batch_data
        word_emb = word_emb.detach().to(self.device).float()
        pos_ohot = pos_ohot.detach().to(self.device).float()
        motions = motions[...,:263].detach().to(self.device).float()
