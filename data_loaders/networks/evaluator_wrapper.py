...
class EvaluatorModelWrapper(object):
    ...
    def __init__(self, opt):
        ...
        if opt.dataset_name in ['t2m', 'behave']:
            opt.dim_pose = 263
...
def build_evaluators(opt):
    ...
    if opt['dataset_name'] in ['humanml' , 'behave', 'omomo']:
        ckpt_dir = 'checkpoints/t2hoi'

    print(f"============ loading behave evaluator checkpoints =====================")
    checkpoint = torch.load(pjoin(opt['checkpoints_dir'], ckpt_dir, 'omomo_hoi', 'model', 'latest.tar'),
                            map_location=opt['device'])
...
class EvaluatorMDMWrapper(object):
    ...
    def __init__(self, dataset_name, use_global, device):
        opt = {
