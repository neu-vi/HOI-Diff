...
def build_models(opt):
    ...
    # Loading pretrained evaluator weight
    checkpoint = torch.load(pjoin('./t2m', 'text_mot_match', 'model', 'finest.tar'),
                            map_location=opt.device)

    movement_enc.load_state_dict(checkpoint['movement_encoder'])
    text_enc.load_state_dict(checkpoint['text_encoder'])
    motion_enc.load_state_dict(checkpoint['motion_encoder'])
    print('Loading Evaluation Model (Epoch %d) Completed!!' % (checkpoint['epoch']))
...
if __name__ == '__main__':
    ...
    if opt.dataset_name == 't2m':
        ...
        opt.data_root = '/work/vig/xiaogangp/codes/guided-motion-diffusion/dataset/Behave_local'
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs_local')
        opt.afford_dir = '/work/vig/xiaogangp/codes/hoi-motion_global/contact_labels2'
        opt.text_dir = pjoin(opt.data_root, 'texts')
        opt.joints_num = 22
        opt.max_motion_length = 196
        opt.dim_pose = 263
        opt.use_global = False
        num_classes = 200 // opt.unit_length
        meta_root = pjoin('./t2m', 'Comp_v6_KLD01', 'meta')
