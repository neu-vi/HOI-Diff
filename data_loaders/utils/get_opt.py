...
def get_opt(opt_path, device, use_global=False, wo_obj_motion=False, hml_rep='hml_vec'):
    opt = Namespace()
    opt_dict = vars(opt)
    ...
    if opt.dataset_name == 't2m_behave':
        if hml_rep == 'SMPL':
            ...
            opt.motion_dir = './dataset/raw_behave/'
            opt.dim_pose = 63 + 10 + 3 + 3 + 6
            ...
            opt.joints_num = 22
            opt.max_motion_length = 196
    elif opt.dataset_name == 't2m_omomo':
        opt.data_root = './dataset/omomo_t2m/'
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs_local')
        opt.dim_pose = 263 + 6
        opt.afford_dir = pjoin(opt.data_root,'affordance_data')
        opt.text_dir = pjoin(opt.data_root, 'texts')
