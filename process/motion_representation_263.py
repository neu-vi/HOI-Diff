...
''' HumanML3D Motion Representation for HOI data
'''
def parse_args():
...
if __name__ == "__main__":
    ...
    joints_num = 22
    # ds_num = 8
    data_dir = pjoin('./dataset', args.dataset+'_t2m', 'sequences')
    data_obj_dir = pjoin('./dataset', args.dataset+'_t2m', 'object_mesh')
    save_dir = pjoin('./dataset', args.dataset+'_t2m', 'sequences_263_rep')
    ...
    example_data = np.load(pjoin(data_dir, example_id, 'human_motion.npz'))['jts']
    ...
    for source_file in tqdm(source_list):
        human_motion = np.load(pjoin(data_dir, source_file, 'human_motion.npz'))['jts'][:, :joints_num]
        source_data_obj = np.load(pjoin(data_dir, source_file, 'object_motion.npz'))
        obj_motion = np.concatenate([source_data_obj['angles'], source_data_obj['trans']], -1)
        try:
            ### compute absoluate root information instead of relative, ignore rec_ric_data
            data, ground_positions, positions, l_velocity, data_obj = process_file(human_motion, obj_motion, 0.002)
            ...
            os.makedirs(pjoin(save_dir, source_file), exist_ok=True)
            np.save(pjoin(save_dir, source_file, 'hoi_motion.npy'), data)
