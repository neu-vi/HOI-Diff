...
def recover_obj_points(obj_motion, temp_points):
    # obj_motion  [T, 3]
    angle, trans = obj_motion[:, :3].transpose(1,0), obj_motion[:, 3:].transpose(1,0)
    rot = Rotation.from_rotvec(angle.transpose(1, 0)).as_matrix()
    obj_points = np.matmul(temp_points[np.newaxis], rot.transpose(0, 2, 1)[:, np.newaxis])[:, 0] + trans.transpose(1, 0)[:, np.newaxis]
