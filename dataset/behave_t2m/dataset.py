...
from data_loaders.behave.data.skel_vis import plot
    ...
    def __init__(self, opt, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list):
            try:
                ...
                obj_sample_path = 'path_object_sample/{}.npy'.format(name)
                if not os.path.exists(obj_sample_path):
                    replace = False if len(obj_points) > 512 else True
                    o_choose = np.random.choice(len(obj_points), 512, replace=replace)
                    o_choose = np.sort(o_choose)
                    np.save(obj_sample_path, o_choose)
                else:
                    o_choose = np.load(obj_sample_path)
                
                # load obj points----------------
                obj_name = name.split('_')[2]
                obj_path = 'path_object_mesh'
                mesh_path = os.path.join(obj_path, simplified_mesh[obj_name])
                temp_simp = trimesh.load(mesh_path)

                obj_points = temp_simp.vertices
                obj_faces = temp_simp.faces
                ...
                contact_data = np.load(pjoin('path_affordance_data', name + '.npy'), allow_pickle=True)[None][0]
                obj_contact_points = contact_data['obj_contact_points'][np.newaxis,:].reshape(1,-1)
                skel_contact_labels = contact_data['skel_contact_labels'][np.newaxis,:].reshape(1, -1)
                contact_input = np.concatenate([obj_contact_points, skel_contact_labels], -1)
...
class TextOnlyAffordDataset(data.Dataset):
    def __init__(self, opt, split_file):
        ...
        for name in tqdm(id_list):
            ...
            # load obj points----------------
            obj_name = name.split('_')[2]
            mesh_path = os.path.join(self.obj_path, simplified_mesh[obj_name])
            temp_simp = trimesh.load(mesh_path)
            ...
            # sample object points
            obj_sample_path = 'path_affordance_data/{}.npy'.format(name)
            if not os.path.exists(obj_sample_path):
                replace = False if len(obj_points) > 512 else True
                o_choose = np.random.choice(len(obj_points), 512, replace=replace)
                o_choose = np.sort(o_choose)
            ...
            text_data = []
            flag = False
            with cs.open(pjoin('path_text_description', name + '.txt')) as f:
                for line in f.readlines():
                    text_dict = {}
                    line_split = line.strip().split('#')
                    caption = line_split[0]
    ...
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list):
            try:
                ...
                obj_points = obj_points[o_choose] 
                obj_normals = obj_normals[o_choose] 
                ...
                motion = motion[:, :-6] if opt.wo_obj_motion else motion
     

                contact_data = np.load(pjoin(opt.afford_dir, name + '.npy'), allow_pickle=True)[None][0]
                obj_contact_points = np.array(contact_data['obj_contact_points'])[np.newaxis,:].reshape(1,-1)
                skel_contact_labels = contact_data['skel_contact_labels'][np.newaxis,:].reshape(1, -1)
                gt_afford_labels = np.concatenate([obj_contact_points, skel_contact_labels], -1)
                ...
                text_data = []
                flag = False
                with cs.open(pjoin('path_text_desctiption', name + '.txt')) as f:
                    for line in f.readlines():
                        text_dict = {}
                        line_split = line.strip().split('#')
                        caption = line_split[0]
                        ...
                        if f_tag == 0.0 and to_tag == 0.0:
                            ...
                            try:
                                ...
                                data_dict[name] = {'motion': n_motion,
                                                    ...
                                                    'gt_afford_labels': gt_afford_labels
                                                }
                                new_name_list.append(name)
                                length_list.append(len(n_motion))
                            except:
    ...
    def __getitem__(self, item):
        ...
        if not self.opt.use_global:
            ...
            if len(self.mean) == 269:
                motion = (motion - self.mean[:motion.shape[-1]]) / self.std[:motion.shape[-1]]
            else:
...
class TextOnlyDataset(data.Dataset):
    def __init__(self, opt, mean, std, split_file):
        ...
        self.normal_dim = opt.dim_pose
        self.obj_path = 'path_object_mesh'
        ...
        for name in tqdm(id_list):
            try:
                ...
                text_data = []
                flag = False
                with cs.open(pjoin('path_text_descriptions', name + '.txt')) as f:
                    for line in f.readlines():
                        text_dict = {}
                        line_split = line.strip().split('#')
                        caption = line_split[0]
    ...
    def __getitem__(self, item):
        ...
        text_list, seq_name, obj_points, obj_normals, gt_afford_labels = data['text'],  data['seq_name'],  data['obj_points'], data['obj_normals'], data['gt_afford_labels']

        # Randomly select a caption
        text_data = random.choice(text_list)
        caption, tokens = text_data['caption'], text_data['tokens']
        return None, None, caption, None, np.array([0]), self.fixed_length, None, seq_name, obj_points, obj_normals, gt_afford_labels
        ...
        if  self.training_stage==1:
            ...
            if self.use_global:
                if mode == 'gt':
                    ...
                    # used by our models
                    self.mean = np.load(pjoin('/work/vig/xiaogangp/codes/guided-motion-diffusion/dataset/Behave_global', 'Mean_global.npy'))
                    self.std = np.load(pjoin('/work/vig/xiaogangp/codes/guided-motion-diffusion/dataset/Behave_global', 'Std_global.npy'))
                ...
                self.mean_rel = np.load(pjoin(opt.data_root, 'Mean_local.npy'))
                self.std_rel = np.load(pjoin(opt.data_root, 'Std_local.npy'))
            else:
                if mode == 'gt':
                    # used by T2M models (including evaluators)
                    self.mean = np.load(pjoin(opt.meta_dir, f't2m_mean.npy'))
                    ...
                    # used by our models
                    self.mean = np.load(pjoin('/work/vig/xiaogangp/codes/guided-motion-diffusion/dataset/Behave_local', 'Mean_local.npy'))
                    self.std = np.load(pjoin('/work/vig/xiaogangp/codes/guided-motion-diffusion/dataset/Behave_local', 'Std_local.npy'))
                ...
                if mode == 'eval':
                    ...
                    self.mean_for_eval = np.load(pjoin(opt.meta_dir, f't2m_mean.npy'))
                    self.std_for_eval = np.load(pjoin(opt.meta_dir, f't2m_std.npy'))
                    # self.mean_for_eval = np.load(pjoin('/work/vig/xiaogangp/codes/guided-motion-diffusion/dataset/Behave_local', 'Mean_local.npy'))
                    # self.std_for_eval = np.load(pjoin('/work/vig/xiaogangp/codes/guided-motion-diffusion/dataset/Behave_local', 'Std_local.npy'))
...
def text_to_object(text):
    obj_list = ['backpack','basketball','boxlarge','boxtiny','boxlong','boxsmall','boxmedium','chairblack','chairwood',
        'monitor','keyboard','plasticcontainer','stool','tablesquare','toolbox','suitcase','tablesmall','yogamat','yogaball','trashbin']
